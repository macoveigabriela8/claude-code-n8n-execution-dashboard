-- Migration: Make deployment_date optional for ROI calculations
-- This allows workflows to be configured without a deployment date,
-- but ROI calculations will not run until deployment_date is set.

-- 1. Make deployment_date nullable
ALTER TABLE public.n8n_workflow_roi 
ALTER COLUMN deployment_date DROP NOT NULL;

-- 2. Update view: vw_workflow_roi_calculated
-- Filter out workflows with NULL deployment_date from ROI calculations
...
SELECT 
    ...
    -- Implementation cost (one-time, if implementation_date is set and passed)
    CASE 
